...
export function ProviderMark({
  ...
  provider: 'openai' | 'cursor' | 'openrouter' | 'claude';
}) {
  return (
    <span className={styles.mark} aria-hidden="true">
      {provider === 'cursor' || provider === 'claude' ? (
        <img src={provider === 'claude' ? claude : cursor} alt="" />
      ) : (
        <>
          <img
            className={styles.light}
            src={provider === 'openrouter' ? openrouterLight : openaiLight}
            alt=""
          />
          <img
            className={styles.dark}
            src={provider === 'openrouter' ? openrouterDark : openaiDark}
            alt=""
          />
        </>
      )}
